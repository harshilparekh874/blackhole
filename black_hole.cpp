...
struct Ray;
bool Gravity = false;
float spinParam = 0.0f;
float renderScale = 0.5f;
bool dynamicRes = true;
int renderWidth = 400;
...
struct Camera {
  ...
  void processKey(int key, int scancode, int action, int mods) {
    ...
    if (action == GLFW_PRESS) {
      ...
      if (key == GLFW_KEY_1)
        renderScale = glm::clamp(renderScale - 0.1f, 0.1f, 1.0f);
      if (key == GLFW_KEY_2)
        renderScale = glm::clamp(renderScale + 0.1f, 0.1f, 1.0f);
      if (key == GLFW_KEY_3)
        spinParam = glm::clamp(spinParam - 0.1f, -0.999f, 0.999f);
      if (key == GLFW_KEY_4)
        spinParam = glm::clamp(spinParam + 0.1f, -0.999f, 0.999f);
      if (key == GLFW_KEY_5)
        dynamicRes = !dynamicRes;

      cout << "[INFO] Scale: " << renderScale << " | Spin: " << spinParam
           << " | Performance: " << (dynamicRes ? "ON" : "OFF") << endl;
    }
  }
};
Camera camera;
...
struct Engine {
  ...
  Engine() {
    ...
    // Setup Camera Callbacks BEFORE ImGui so ImGui can chain them
    ...
    for (int z = 0; z <= gridSize; ++z) {
      for (int x = 0; x <= gridSize; ++x) {
        ...
        for (const auto &obj : objects) {
          ...
          // prevent sqrt of negative or divide-by-zero (inside or at the
          // black hole center)
          if (dist > r_s) {
  ...
  void uploadCameraUBO(const Camera &cam) {
    ...
    data.aspect = float(renderWidth) / float(renderHeight);
    data.moving = cam.dragging || cam.panning;
    data.screenWidth = renderWidth;
    data.screenHeight = renderHeight;
    data.spin = spinParam;
...
// -- MAIN -- //
int main() {
  // setupCameraCallbacks already called in Engine constructor
  ...
  while (!glfwWindowShouldClose(engine.window)) {
    ...
    // UI
    {
      ...
      ImGui::Checkbox("Performance Mode", &dynamicRes);
      ImGui::Separator();
      ImGui::SliderFloat("Black Hole Spin", &spinParam, -0.999f, 0.999f);
      ImGui::Checkbox("N-Body Gravity", &Gravity);
      ImGui::Separator();
      ImGui::Text("Internal Res: %dx%d", renderWidth, renderHeight);
    ...
    mat4 proj = perspective(radians(60.0f), float(renderWidth) / renderHeight,
                            1e9f, 1e14f);
    ...
    // ---------- RUN RAYTRACER ------------- //
    int curWinW, curWinH;
    glfwGetFramebufferSize(engine.window, &curWinW, &curWinH);

    // Scale resolution based on window size
    renderWidth = (int)(curWinW * renderScale);
    renderHeight = (int)(curWinH * renderScale);

    int oldW = renderWidth;
    int oldH = renderHeight;
    if (dynamicRes && (camera.dragging || camera.panning)) {
      renderWidth /= 2;
      renderHeight /= 2;
    }

    glViewport(0, 0, curWinW, curWinH);
